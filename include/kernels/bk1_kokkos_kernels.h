...
namespace BK1
{
  namespace Parallel
  {
...

    {
      ...
      {
        // std::cout << "Start testing BK1 " << std::endl << std::endl;
            ...
            while (cell_index < n_cells)
              {
                ...
                for (unsigned int tid = threadIdx; tid < nm_coarse_total;
                     tid += blockSize)
                  {
                    // const unsigned int i = tid / (nm_coarse * nm_coarse);
                    ...
                    // std::cout << s_wsp0[shared_idx] << "   ";

                    // std::cout << s_wsp0[shared_idx] << "   ";
                  }
                team_member.team_barrier();

                // std::cout << "cell_id = " << cell_index << ": ";
                // for (unsigned int i = 0; i < nm_coarse_total; ++i)
                //   std::cout << s_wsp0[i] << "   ";
                ...
                // std::cout << "cell_id = " << cell_index << ": ";
                // for (unsigned int i = 0; i < nm_fine * nm_coarse * nm_coarse;
                //      ++i)
                //   std::cout << s_wsp1[i] << "   ";
                // std::cout << std::endl << std::endl;

                // std::cout << "Finished direction 0 " << std::endl;
                ...
                // std::cout << "cell_id = " << cell_index << ": ";
                // for (unsigned int i = 0; i < nm_coarse * nm_fine * nm_fine;
                // ++i)
                //   std::cout << s_wsp0[i] << "   ";
                // std::cout << std::endl << std::endl;


                // std::cout << "Finished direction 1 " << std::endl;
                ...
                // std::cout << "cell_id = " << cell_index << ": ";
                // for (unsigned int i = 0; i < nm_fine * nm_fine * nm_fine;
                // ++i)
                //   std::cout << s_wsp1[i] << "   ";
                // std::cout << std::endl << std::endl;


                // std::cout << "Finished direction 2 " << std::endl;


                // // Reverse Operations

                // // step-6 : direction 2
                // for (int tid = threadIdx; tid < c_nelmtPerBatch * nq * nq;
                //      tid += blockSize)
                //   {
                //     int b = tid / (nq * nq);
                //     int q = tid % (nq * nq) / nq;
                ...
                //       {
                //         reg[r] =
                //           s_wsp1[b * nq * nq * nq + r * nq * nq + q * nq +
                //           p];
                //       }
                ...
                //         s_wsp0[b * nm * nq * nq + k * nq * nq + q * nq + p] =
                //           tmp;
                //       }
                //   }
                // team_member.team_barrier();
                ...
                // // step-7 : direction 1
                // for (int tid = threadIdx; tid < c_nelmtPerBatch * nm * nq;
                //      tid += blockSize)
                //   {
                //     int b = tid / (nm * nq);
                //     int k = tid % (nm * nq) / nq;
                ...
                //       {
                //         reg[q] =
                //           s_wsp0[b * nm * nq * nq + k * nq * nq + q * nq +
                //           p];
                //       }
                ...
                //           }
                //         s_wsp1[b * nm * nm * nq + k * nm * nq + j * nq + p] =
                //           tmp;
                //       }
                //   }
                // team_member.team_barrier();
                ...
                // // step-8 : direction 0
                // for (int tid = threadIdx; tid < c_nelmtPerBatch * nm * nm;
                //      tid += blockSize)
                //   {
                //     int b = tid / (nm * nm);
                //     int j = tid % (nm * nm) / nm;
                ...
                //       {
                //         reg[p] =
                //           s_wsp1[b * nm * nm * nq + k * nm * nq + j * nq +
                //           p];
                //       }
                ...
                //           }
                //         s_wsp0[b * nm * nm * nm + i * nm * nm + j * nm + k] =
                //           tmp;
                //       }
                //   }
                // team_member.team_barrier();
                ...
                for (unsigned int tid = threadIdx;
                     ...
                     tid += blockSize)
                  {
                    // const unsigned int i = tid / (nm_fine * nm_fine);
                    // const unsigned int j =
                    //   (tid % (nm_fine * nm_fine)) / nm_fine;
                    // const unsigned int k = tid % nm_fine;

                    {
                      ...
                      // const unsigned int shared_idx =
                      //   e * nm_fine_total + local_idx;

                      //   apply weights
                      Number value_out = weights(tid, cell_index) * s_wsp1[tid];

                      //   Number value_out = s_wsp1[tid];
                ...
                //   std::cout << "cell_id = " << cell_index << ": ";
                //   for (unsigned int i = 0; i < nm_fine * nm_fine *
                //   nm_fine;
                //        ++i)
                //     std::cout << s_wsp1[i] << "   ";
                //   std::cout << std::endl << std::endl;
