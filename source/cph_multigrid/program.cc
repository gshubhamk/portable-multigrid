...
private:
  ...
  MGLevelObject<std::unique_ptr<FiniteElement<dim>>> p_level_fes;
...
LaplaceProblem<dim, fe_degree>::setup_dofs()
{
  ...
  for (unsigned int level = level_dof_handlers.min_level(); level <= level_dof_handlers.max_level();
       ++level)
    {
  ...
  pcout << " Total number of levels " << level_dof_handlers.max_level() + 1
        << ": h_levels = " << coarse_triangulations.size() << ", p_levels = " << p_levels.size() - 1
        << ", c_level = " << 1 << std::endl;
  ...
  for (unsigned int level = level_dof_handlers.min_level(); level <= level_dof_handlers.max_level();
       ++level)
    {
      if (level < coarse_triangulations.size())
        ...
        {
          pcout << "p_level = " << p_level_fes[level + 1 - coarse_triangulations.size()]->degree
                << ": n_dofs = ";
          pcout << level_dof_handlers[level].n_dofs() << std::endl;
        }
      else
        {
          pcout << "c_level = " << fe_degree << ": n_dofs = ";
          pcout << level_dof_handlers[level].n_dofs() << std::endl;
        }
    }
  pcout << std::endl;
...
LaplaceProblem<dim, fe_degree>::setup_matrix_free()
{
  ...
  for (unsigned int level = 0; level <= level_dof_handlers.max_level(); ++level)
    {
      if (level < coarse_triangulations.size())
        level_matrices[level] = std::make_unique<Portable::LaplaceOperator<dim, 1, double>>(
          level_dof_handlers[level], level_constraints[level], overlap_communication_computation);
       ...
       ++level)
    {
      if (level < coarse_triangulations.size())
      ...
      else if (level < level_matrices.max_level())
        {
          ...
          const unsigned int p_fine = p_level_fes[level + 1 - coarse_triangulations.size()]->degree;
      ...
      if (cell->is_locally_owned())
        {
          ...
          constraints_fine.distribute_local_to_global(cell_rhs, local_dof_indices, system_rhs_host);
        }
    }
...
LaplaceProblem<dim, fe_degree>::solve()
{
  ...
  Portable::VCycleMultigrid<dim, double, Portable::MGTransferBase<dim, double>> mg_preconditioner(
    level_matrices, mg_transfers, mg_smoothers);
  ...
  cg.solve(system_matrix, solution_device, system_rhs_device, mg_preconditioner);
...
void
solve_for_degree(int fe_degree, const unsigned int n_pre_smooth, const unsigned int n_post_smooth)
{
  if (degree == fe_degree)
    {
      LaplaceProblem<dim, degree> laplace_problem(n_pre_smooth, n_post_smooth, false);
      laplace_problem.run();
    }
