...
namespace multigrid
{
  ...
  LaplaceProblem<dim, fe_degree>::compute_rhs()
  {
    ...
    op_bk3->compute_rhs(system_rhs_device, 1.0); // Call the GPU kernel to compute the right-hand side vector on device memory
    ...
    for (unsigned int cycle = 0; cycle < sizeof(sizes) / sizeof(unsigned int); ++cycle)
      {
        ...
        pcout << std::endl;
        pcout << "----------------------------------" << std::endl;
        pcout << "Timings computed using old method (vmult_dummy)" << std::endl;
        matvec_ghost_timing();
        pcout << "----------------------------------" << std::endl;
