...
namespace multigrid
{
  ...
  private:
    ...
    LinearAlgebra::distributed::Vector<full_number, MemorySpace::Host>
      ghost_solution_host;
    LinearAlgebra::distributed::Vector<full_number, MemorySpace::Default>
      solution_device;
    LinearAlgebra::distributed::Vector<full_number, MemorySpace::Default>
      system_rhs_device;
                                        ...
                                        const unsigned int n_post_smooth)
  {
    ...
    if constexpr (std::is_same_v<full_number, vcycle_number>)
      solver =
        new multigrid::MultigridSolver<dim,
                                       fe_degree,
                                       vcycle_number,
                                       full_number,
                                       SmootherType>(level_matrices.back(),
                                                     level_dof_handlers,
                                                     level_constraints,
                                                     mg_transfers,
                                                     level_matrices,
      ...
      solver = new multigrid::MultigridSolver<dim,
                                              fe_degree,
                                              vcycle_number,
                                              full_number,
                                              SmootherType>(fine_level_matrix,
                                                            level_dof_handlers,
                                                            level_constraints,
                                                            mg_transfers,
                                                            level_matrices,
  ...
  try
    {
      ...
      std::size_t  minsize           = 100000000;
      unsigned int n_pre_smooth      = 5;
      unsigned int n_post_smooth     = 5;
